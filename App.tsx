...
import { mockEvents, mockUsers, mockVenues, mockDocuments, mockBands, mockFolders } from './lib/mockData';
import type { Event, User, Venue, Document, Band, Folder, Notification } from './types';
...
export const AppContext = React.createContext<{
  ...
  events: [], users: [], allUsers: [], venues: [], documents: [], folders: [], bands: [], notifications: [],
...
const App: React.FC = () => {
  ...
  // --- NOTIFICATIONS ---
  const addNotification = (userIds: string[], message: string) => {
  ...
  const addEvent = (event: Omit<Event, 'id' | 'bandId'>) => {
    ...
    const userIdsToNotify = newEvent.assignments.map(a => a.userId);
    addNotification(userIdsToNotify, `Nieuw event: Je bent toegevoegd aan "${newEvent.title}".`);
  };
  ...
  const updateEvent = (eventToUpdate: Event) => {
    ...
    const userIdsToNotify = eventToUpdate.assignments.map(a => a.userId);
    addNotification(userIdsToNotify, `Update: Details voor "${eventToUpdate.title}" zijn gewijzigd.`);
  }
  ...
  const updateUser = (userToUpdate: User) => {
    ...
    // If the user being updated is the current user, update their session state.
    if (userToUpdate.id === currentUser?.id) {
        setCurrentUser(userToUpdate);
    }
    
    if (originalUser) {
        if (originalUser.bandId !== userToUpdate.bandId) {
             // Handled in BandManagementPage toast
  ...
  const deleteUser = (userId: string) => {
    ...
    const isAssigned = allEvents.some(e => 
      e.assignments.some(a => a.userId === userId) ||
      e.callSheet?.travelParty?.some(tp => tp.userId === userId)
    );

    if (isAssigned) {
      toast({ title: 'Verwijderen Mislukt', description: `${userToDelete?.name} is nog toegewezen aan een of meerdere events en kan niet worden verwijderd.`, variant: 'destructive' });
      return;
    }
    
    if (!window.confirm(`Weet je zeker dat je ${userToDelete?.name} permanent wilt verwijderen? Dit kan niet ongedaan gemaakt worden.`)) return;

    setAllUsers(prev => prev.filter(user => user.id !== userId));
    
    // Also remove from callsheet tourmanager etc.
    setAllEvents(prev => prev.map(event => {
        let newCallSheet = event.callSheet;
        if (event.callSheet?.tourManagerId === userId) {
            newCallSheet = { ...event.callSheet, tourManagerId: undefined };
        }
        return { ...event, callSheet: newCallSheet };
    }));
  ...
  const contextValue = {
    events, users, allUsers, venues, documents, folders, notifications,
