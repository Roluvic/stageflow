...
import type { Venue, Event, User } from '../../types';
...
const EventsList: React.FC<{ events: Event[], venues: Venue[], onEventClick: (event: Event) => void, canEdit: boolean, onDeleteEvent: (eventId: string) => void }> = ({ events, venues, onEventClick, canEdit, onDeleteEvent }) => {
    return (
        <div className="space-y-2">
            {events.map(event => {
                const venue = venues.find(v => v.id === event.venueId);
                return (
                    <Card key={event.id} className="rounded-xl group cursor-pointer hover:bg-accent transition-colors" onClick={() => onEventClick(event)}>
                        <CardContent className="p-6 relative">
                            ...
                            {canEdit && (
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button size="icon" variant="destructive" className="h-8 w-8 rounded-full" onClick={(e) => { e.stopPropagation(); onDeleteEvent(event.id); }} title="Verwijder event">
...
export const VenuesPage: React.FC = () => {
  const { venues, addVenue, updateVenue, deleteVenue, currentUser, addEvent, deleteEvent } = useContext(AppContext);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <div className="space-y-4 max-h-[calc(65vh-60px)] overflow-y-auto pr-2">
                        {activeTab === 'events' && <EventsList events={filteredAndSortedEvents} venues={venues} onEventClick={setSelectedEvent} canEdit={canEdit} onDeleteEvent={deleteEvent} />}
                        {activeTab === 'venues' && (
                             <div className="space-y-2">
                                        ...
                                        <CardContent className="p-6 flex items-center justify-between">
                                            ...
                                            {canEdit && (
                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <Button size="icon" variant="destructive" className="h-8 w-8 rounded-full" onClick={(e) => { e.stopPropagation(); handleDeleteVenue(venue.id); }}><Trash2 className="h-4 w-4" /></Button>
                                                </div>
                                            )}
                                        </CardContent>
                                    </Card>
